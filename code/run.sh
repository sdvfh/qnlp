...
for dataset in "${autogenerated_datasets[@]}"; do
  for classifier in "${quantum_classifiers[@]}"; do
    for layers in 1 10; do
      python experiments.py -dataset "$dataset" -model_transformer "all-mpnet-base-v2"                   -n_features 768 --n_qubits 10 -model_classifier "$classifier" --epochs 40 --batch_size 20 --n_repetitions 30 --n_layers "$layers" ; \
      python experiments.py -dataset "$dataset" -model_transformer "all-mpnet-base-v2"                   -n_features 32  --n_qubits 5  -model_classifier "$classifier" --epochs 40 --batch_size 20 --n_repetitions 30 --n_layers "$layers" ; \
